...
public class ActiveRecord
{
            ...
            if (!row.isEmpty()) {
                ...
                row.forEach((c, v) -> {
                    if (this.getColumns().containsKey(c)) {
                        field[0] = (String)this.getColumns().get(c).get("field");
                        this.setFieldValue(field[0], v);
                    }
                });
            }
        }
    ...
    public void unLoad() {
        ...
        this.columns.forEach((columnName, column) -> {
            if (columnName != null && column != null && !column.isEmpty()) {
                ...
                value[0] = o2[0];
                this.setFieldValue(String.valueOf(column.get("field")), value[0]);
            }
        });
    }
    ...
    public int insert() {
        ...
        try {
            ...
            while (it.hasNext()) {
                ...
                final String columnName = (String) pair.getKey();
                final LinkedHashMap<String, Object> column = (LinkedHashMap<String, Object>) pair.getValue();
                if (column.get("primary") != null) {
    ...
    public int update() {
        ...
        try {
            final String primaryColumn = (String) this.primary.get("name");
            final Object primaryValue = this.getFieldValue((String)this.primary.get("field"));
    ...
    public int sync() {
        ...
        try {
            ...
            while (it.hasNext()) {
                ...
                final boolean primary = (boolean) column.get("primary");
                final boolean autoIncrement = (boolean) column.get("autoincrement");
                final String type = (String) column.get("type");
                final String nullable = (boolean) column.get("nullable") ? " DEFAULT NULL " : " NOT NULL ";
                final int length = (int) column.get("length");
                final String s = type;
                switch (s) {
                    case "integer": {
    ...
    public static ActiveRecord first(final Class subClass, final String condition, final Object[] values) {
        ...
        try {
            final ActiveRecord outerObject = (ActiveRecord) subClass.getConstructor((Class<?>[])new Class[0]).newInstance(new Object[0]);
            ...
            if (row != null && !row.isEmpty()) {
                record = (ActiveRecord) subClass.getConstructor((Class<?>[])new Class[0]).newInstance(new Object[0]);
