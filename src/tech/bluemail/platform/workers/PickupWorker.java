...
public class PickupWorker extends Thread
{
    ...
    public void run() {
        try {
            ...
            if (this.emails != null && !this.emails.isEmpty() && !this.drop.isStoped) {
                ...
                final String messageId = "<" + messageIdBuilder.append(messageIdBuilder.hashCode()).append('.').append(Strings.getSaltString(13, false, false, true, false)).append('.').append(System.currentTimeMillis()).append('@').append(vmta.domain).toString() + ">";
                final String mailDate = new SimpleDateFormat("EEE, dd MMM yyyy HH:mm:ss Z").format(new Date());
                final String globalBounceEmail = this.replaceTags(this.drop.bounceEmail, vmta, email, mailDate, messageId, this.drop.staticDomain, this.drop.id, this.drop.mailerId);
                ...
                final String header = new String(String.valueOf(DropsSender.getCurrentHeader()).getBytes());
                ...
                if (!"".equals(body)) {
                    final String contentTransferEncoding = this.drop.contentTransferEncoding;
    ...
    public synchronized void createMailMerge(final Fresh email, final Vmta vmta, final StringBuilder pickup) throws Exception {
        if (email != null && !"".equals(email.email.trim()) && vmta != null) {
            ...
            final String messageId = "<" + messageIdBuilder.append(messageIdBuilder.hashCode()).append('.').append(Strings.getSaltString(13, false, false, true, false)).append('.').append(System.currentTimeMillis()).append('@').append(vmta.domain).toString() + ">";
            final String mailDate = new SimpleDateFormat("EEE, dd MMM yyyy HH:mm:ss Z").format(new Date());
            final String bounceEmail = this.replaceTags(this.drop.bounceEmail, vmta, email, mailDate, messageId, this.drop.staticDomain, this.drop.id, this.drop.mailerId);
            final String returnPath = this.replaceTags(this.drop.returnPath, vmta, email, mailDate, messageId, this.drop.staticDomain, this.drop.id, this.drop.mailerId);
            final String fromEmail = this.replaceTags(this.drop.fromEmail, vmta, email, mailDate, messageId, this.drop.staticDomain, this.drop.id, this.drop.mailerId);
            final String replyTo = this.replaceTags(this.drop.replyTo, vmta, email, mailDate, messageId, this.drop.staticDomain, this.drop.id, this.drop.mailerId);
            final String to = this.replaceTags(this.drop.to, vmta, email, mailDate, messageId, this.drop.staticDomain, this.drop.id, this.drop.mailerId);
            final String received = StringUtils.replace(this.replaceTags(this.drop.received, vmta, email, mailDate, messageId, this.drop.staticDomain, this.drop.id, this.drop.mailerId), "[return_path]", returnPath);
            final String fromName = this.replaceTags(this.drop.fromName, vmta, email, mailDate, messageId, this.drop.staticDomain, this.drop.id, this.drop.mailerId);
            final String subject = this.replaceTags(this.drop.subject, vmta, email, mailDate, messageId, this.drop.staticDomain, this.drop.id, this.drop.mailerId);
            ...
            if (this.drop.randomTags != null && this.drop.randomTags.length > 0) {
                for (final String randomTag : this.drop.randomTags) {
